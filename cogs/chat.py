...
class Chat(commands.Cog):
    def __init__(self, bot):
        ...
        self.valid_bot = ["Concrete Test", self.bot.bot_name]
        # special actions
        self.actions = {
    ...
    def phrase_matcher(self, phrase):
        ...
        for intent in self.phrase_data["intents"]:
            for pattern in intent["patterns"]:
                ...
                similarity = difflib.SequenceMatcher(
                    None, prepped_pattern, prepped_phrase
                ).ratio()
        ...
        if self.debug:
            print(phrase)
            print(
                f'Final pick is: {best_response["tag"]} with similarity: {max_similarity}\n{matched_pattern}\n'
            )
        ...
        if self.respond_if(message):
            ...
            if len(intent["responses"]) > 1:
                if "weighted" in intent.keys():
                    response = random.choices(
                        intent["responses"], weights=(intent["weighted"])
                    )[0]
                else:
            ...
            if "{" in response:
                replacements = {
                    ...
                    "{years_old}": str(
                        self.bot_func.readable_time_since(
                            dt.datetime(2020, 2, 18, 0, 0, 0, 0)
                        )
                    ),
                }
