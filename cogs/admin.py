...
class Admin(commands.Cog):
    ...
    @commands.command(name="clear", brief="Deletes 200 messages.")
    @commands.has_guild_permissions(manage_messages=True)
    ...
    @commands.command(name="reload", brief="Reloads all cogs")
    ...
    async def changeactivity(self, ctx, *activity):
        ...
        await self.bot.change_presence(
            activity=ds.Activity(type=ds.ActivityType.playing, name=activity)
        )
        await ctx.message.delete()

    @commands.command(
        name="backup", brief="Backs up log files and sends attaches them to a message."
    )
    @commands.is_owner()
    async def backup(self, ctx):
        ...
        if os.path.exists(source_dir):
            ...
            await ctx.send(
                f"{os.path.basename(source_dir)} does not exist. Backup has been cancelled."
            )

    @commands.command(
        name="pid", brief="Sends current bot PID.", description="Sends current bot PID."
    )
    @commands.is_owner()
