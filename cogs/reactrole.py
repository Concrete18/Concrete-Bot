...
class ReactRole(commands.Cog):
        ...
        if not payload.member.bot:
            ...
            for item in data:
                if item["emoji"] == payload.emoji.name:
                    role = ds.utils.get(
                        self.bot.get_guild(payload.guild_id).roles, id=item["role_id"]
                    )
    ...
    async def on_raw_reaction_remove(self, payload):
        """
        Detects removed reactions and removes the role for the specific
        reaction if it is in the reactrole.json.
        """
        with open("Logs/reactrole.json") as react_file:
            data = json.load(react_file)
        for item in data:
    ...
    async def on_message_delete(self, message):
        """
        On message delete in the react role channel, the entry for the role
        reaction in the json will also be deleted.
        """
        if "reaction to get the" in message.clean_content:
            print("React Role was deleted.")
            self.bot.logger.info(
                f"Deleted {print(message.id)} React Role. Check the data from the json."
            )
