...
class RPG(commands.Cog):
    ...
    @commands.command(
        ...
        help="""
        Just use the /groupsplit then type your totals of Plat, Gold, Silver
        and Copper in any order. If you do not specify a split using (n)way,
        the split will default to 4.
        ...
        The above 2 command detects 20 gold, 14 copper, and 20 silver. Spaces
        and full coin names are optional. This works the same way and both are
        a 5 way split.

        You should notice that it can accept more then one set of each type of
        coin in case you want to add 438 gold from selling some items plus
        the 1250 award for the quest.
        """,
    )
    async def groupsplit(self, ctx, *args):
        ...
        for coin in coins:
            for arg in args:
                if coin in arg.lower():
                    ...
                    coins["way"] = int(arg)
        amounts = f'Platinum: {coins["p"]}'
        amounts += f' | Gold: {coins["g"]}'
        amounts += f' | Silver: {coins["s"]}'
        amounts += f' | Copper: {coins["c"]}'
        # create gold split
        goldtotal = (
            (coins["c"] / 100) + (coins["s"] / 10) + coins["g"] + (coins["p"] * 10)
        )
        ...
        embed.add_field(
            name=f"Split",
            value=f'{coins["way"]} Way',
            inline=False,
        )
        if int(goldsplit) > 0:
            embed.add_field(
                name="Gold",
                value=f"{int(goldsplit)}",
                inline=True,
            )
        if int(silverleft) > 0:
            embed.add_field(
                name="Silver",
                value=f"{int(silverleft)}",
                inline=True,
            )
        if int(copperleft) > 0:
            embed.add_field(
                name="Copper",
                value=f"{int(copperleft)}",
                inline=True,
            )
        if int(extraCopper) > 0:
            embed.add_field(
                name="Extra Copper",
                value=f"{int(extraCopper)}",
                inline=False,
            )
