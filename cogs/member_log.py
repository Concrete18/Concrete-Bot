...
class Member_Log(commands.Cog):
    ...
    async def update_activity(self, member, activity_type):
        ...
        """
        if (
            member.guild.id != self.bot.main_server
            and member.guild.id != self.bot.test_server
        ):
            return
    ...
    async def on_message(self, message):
        """
        Updates last server action to the current date if an action has not
        occurred  on the current day.
        """
    ...
    async def on_voice_state_update(self, member, before, after):
        """
        Updates last server action to the current date if an action
        has not occurred  on the current day.
        """
    ...
    async def showactivemembers(self, ctx, days: int = 1):
        ...
        await ctx.send(
            f"{len(active_list)} active members for the last {days} days:\n{result}"
        )
    ...
    async def showinactivemembers(self, ctx, days: int = 60):
        ...
        await ctx.send(
            f"{len(inactive_list)} inactive members for the last {days} days:\n{result}"
        )

    @commands.command(
        name="updatemembers", brief="Adds missing members to member_data."
    )
    @commands.has_guild_permissions(manage_messages=True)
    async def update_member_data(self, ctx):
        ...
        """
        if (
            ctx.guild.id != self.bot.main_server
            and ctx.guild.id != self.bot.test_server
        ):
            return
        new_members = []
