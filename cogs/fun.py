...
class Fun(commands.Cog):
    ...
    async def say_hello(self, ctx):
        ...
        san = ("", "-san")
        await ctx.send(
            f"{random.choice(hello)} {ctx.author.mention}{random.choices(san, weights=(60, 25))[0]}!"
        )
    ...
    @commands.command(name="rps", brief="Play Rock Paper Scissors.")
    async def rps(self, ctx, player_move):
        ...
        if player_move.lower() not in ["rock", "paper", "scissors"]:
            await ctx.channel.send(
                "Invalid gesture, please use Rock, Paper or Scissors."
            )
            return
        # player
        ...
        embed.add_field(
            name=f"{player_name} Played", value=f"{player_move}", inline=True
        )
